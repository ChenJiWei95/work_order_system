	...
	*/
	...
	表单自定义验证 2018/3/16
	参数：
		isDebug   boolean 是否为调试状态   2018/12/12 调试状态有调试信息输出
	...
	function FormVerifi(data){
		...
		for(var key in data.data){
			...
			(function(ele, key){  //使用闭包保存每个事件的触发元素 使触发独立的
				if(data.isDebug) console.log("验证检测到的元素："+ele.tName());
				//输入的时候检测
				ele.oninput = function(){
					if(data.isDebug) console.log("data."+data.data[key]+"(ele)");
...
/********************************************************************************/
	...
	*/
	...
	function JSONTokener(){
		...
		function _parseObject(str){
			...
			if(c == '['){
				return _dealArray(str.substring(str.indexOf('[') + 1, str.length - 1));
			}else if (c == '{'){
				return _dealObject(str.substring(str.indexOf('{') + 1, str.length - 1));
			}else{
	...
	/**
	 * 更新 
	 * add if(jsonObject.length > 0) 才会被载入 2018/9/15
	 * removeAll 2018/9/16
	 */
	function JSONArray(arrayList){
		var containerlists = (arrayList == undefined 
			? new ArrayList() 
			: arrayList); 
		...
		this.forEach = function (fn){ 
			containerlists.forEach(fn);
		}
		this.subJSONArray = function(formIndex, toIndex){
			var con_ = containerlists.subList(formIndex, toIndex);
			return new JSONArray(con_);
		}
	}
	/**
	 * 更新
	 * this.length 属性改为this.length() 成员方法 解决一直长度为零的问题 2018/9/15
	 */
	function JSONObject(){
		var map = new Map();
		this.length = function(){
	...
	/**
	 * ObjectParseJSON V1 2018/9/14
	 *	将一个原始的json对象装换成封装的JSON对象（JSONObject、JSONArray）
	 * 
	 * mar 3,2019 发现异常 有冲突符存在 已解决
	 *		使用JSONObject的put_方法
	 * 
	 */
	...
	*/
	...
	alias	别名
	action	函数名
	有别名时那么url链接的是别名，没定义别是时action首字母小写作为别名
	@Renew 增加actioConf对象
	*/
	
	var actionConfJson;//var actionConfJson = [{alias:"ac", action:"ActionC"}]; 废弃
	var actioConf;//var actioConf=[{urls:[{url:""}], actionInstance:new 对象()},];
	var actioConfMap;
	//初始化配置和加载配置
	function Configure(){
		if(actioConf != undefined){
			actioConfMap = new Map();
			var actioConfC = $$.json.toJSON(actioConf);
			actioConfC.forEach(function (item){
				var actionInstance = item.get("actionInstance");
				if(actionInstance == undefined){
					$$.error("配置中的actionInstance实例不存在，actionInstance="+actionInstance+"；请检查配置！");
				}
				if(item.get("urls").length() == 0) actioConfMap.put("execute", actionInstance);
				item.get("urls").forEach(function (item){
					actioConfMap.put(item.get("url"), actionInstance);
				});
			});
		}
	}
	...
	/**
	 * ajaxC 模拟请求 2018/9/16
	 * 描述：
	 *	1、使用方式与ajax一样 
	 *	2、url的书写则是目的地Action(此处为模拟Action)
	 *	3、创建模拟Action：需书写函数:
	 *		function ActionC(){
	 *			this.此处为url = function(data){
	 *				//data为ajaxC参数data
	 *				//当前模拟成功响应时data.success("ok"); 反之则data.error("no");
	 *			}
	 *		}
	 *	4、ajaxC范例：
	 *		ajaxC({
	...
	function ajaxC(data){
		//第一个if 废弃
		if(actionConfJson != undefined){
			var actionConfJ = $$.json.toJSON(actionConfJson);
			actionConfJ.forEach(function (item){
				if(item.get("alias") != undefined)
					url = item.get("alias");
				else url = $$.util.firstLowercase(item.get("action"));
				if(data.url == url)
					eval("var action = new "+item.get("action")+"();"+
					"var bool = action.execute(data.data);"+
					"if(bool) "+
						"data.success($$.tempData);"+
					"else "+
						"data.error($$.tempData)");
			});
		}
		if(actioConfMap != undefined && actioConfMap.size() > 0){
			var instance = actioConfMap.get(data.url);
			if(instance != null){
				eval(instance+"."+data.url+"(data.data);");
			}else $$.error(data.url+"链接无效；没有实例化的Action实际对象，请检查配置");
		}
	}
	/**
	 * ajaxC 模拟请求 2018/9/16 历史版本：ajaxTest
	 * 描述：
	 *	1、使用方式与ajax一样 
	 *	2、url的书写则是目的地Action(此处为模拟Action)
	 *	3、创建模拟Action：需书写函数:
	...
	var actionCJW;
	function ajaxC(data){
		if(actionCJW == null) actionCJW = new ActionC();
		eval("actionCJW."+data.url+"(data)");
	}*/
	/**
	 * 
	 * 模拟ajax 对前台进行测试 18-9-12
	 *		successdata 模拟ajax执行成功返回的数据
	 *		faildata	模拟ajax执行失败返回的数据
	 *		isSuccess	设置ajax执行成功的方法还是执行失败的方法
	 * 使用范例
	 *	ajaxTest(
	 *		{
	 *			type:"",
	 ...
	 *		参数2为 测试配置参数
	 */
	function ajaxTest(data, testData){
		//testData.successdata; //模拟后台传回来的成功数剧
		//testData.faildata;	
		//默认调用成功的方法
		if(testData.isSuccess == undefined)
			testData.isSuccess = true; 
		console.log("当前调用的是ajax测试版对前台页面进行测试，此方法不传递到后台，而是由前台自导自演。");
		console.log("type:"+data.type+"; url:"+data.url+"; data:"+data.data);
		//默认调用成功的方法 可以设置调用
		if(testData.isSuccess)
			data.success(testData.successdata);
		else 
			data.error(testData.faildata);
	}
	/**
	 * 
...
/********************************************************************************/
	//获取类型字符串 传入任意类型，返回该类型的字符串
	function classof(o){
		if (o === null) return "Null";
		if (0 === undefined) return "Undefined";
		//因为很多对象继承的toString方法重写了，所以此处调用的是Object.prototype.toString 
		//并且获取第8个位置到倒数第二个的位置的字符串进行返回
...
/********************************************************************************/
	...
	@Param liElements li标签集	
	@Param cntElements	如果点击对应的容器
	@Param initIndex 设置初始元素下标
	@Param isInitRun 设置是否进行初始化
	@Renew 新增 添加初始执行布尔值data.isInitRun 2018/9/19
	@Renew 删除 删除cntElements该（exception.NullException）异常检测,并不是必须的； 2018/9/19 
	@History LiEventSuper 2017/12/29 V1
	*/
	function LiEventSuper(data){
		...
		//是否进行初始化 默认是进行初始化的
		if(data.isInitRun == undefined)
			data.isInitRun = true;
		this.preIndex;	//2018/9/30
		var oneSelf = this;
		//当前移入样式设置
		this.overEventCss = function (i, e){}
		//当前移出样式设置
		this.outEventCss = function (i, e){}
		//e {i:preIndex}
		this.preClickFun = function (i, e){}
		//设置当前的元素 
		//e {i:i}
		this.nowClickFun = function (i, e){}
		//初始化设置预设元素样式
		function _init(i, e){
		...
		/* S - 事件模板 */
		this.overEventFun = function(i, e){
			if(data.liElements[oneSelf.preIndex] != e.target)
				oneSelf.overEventCss(i, e.target);
		}
		this.clickEventFun = function(i, e){
			if(data.liElements[oneSelf.preIndex] != e.target){
				if(data.liElements[oneSelf.preIndex] != undefined)
					oneSelf.preClickFun(oneSelf.preIndex, data.liElements[oneSelf.preIndex]);
				oneSelf.nowClickFun(i, e.target);
				oneSelf.preIndex = i;
			}		
		}
		this.outEventFun = function(i, e){
			if(data.liElements[oneSelf.preIndex] != e.target)
				oneSelf.outEventCss(i, e.target);
		}
		/* E - 事件模板 */
		//arr所有li元素以数组的形式传入
		this.doReady = function (){
			if(data.isInitRun){
				oneSelf.preIndex = data.initIndex;
				_init(data.initIndex, data.liElements[data.initIndex]);
			}	
			...
			for(var i = 0; i < len; i ++){
				...
				(function(i){
					...
					data.liElements[i].onmouseover = function (e){
						oneSelf.overEventFun(i, e);
					}
					//移出事件 不等于上一个点击元素则触发
					data.liElements[i].onmouseout = function (e){
						oneSelf.outEventFun(i, e);
					}
					//点击事件 不等于上一个点击元素则触发
					data.liElements[i].onclick = function (e){
						oneSelf.clickEventFun(i, e);
					}
				})(i);
			}
		}
	}
	//可重复点击同一个组件并作出不同的操作 2018/9/30 
	//dbClickFun 第二次点击
	//
	SuperLiEventCasc.extends(LiEventSuper);
	function SuperLiEventCasc(data){
		LiEventSuper.call(this, data);
		var oneSelf = this;
		//当点击为双击时执行
		this.dbClickFun = function(i, e){
		}
		var isDbclick = true; //是否为第二次击
		//重写点击事件
		this.clickEventFun = function(i, e){
			console.log(oneSelf.preIndex !== i);
			isRepeatClick = oneSelf.preIndex !== i ? true : false;
			if(isRepeatClick){
				//组件被第一次点击
				//调用preClick 恢复上一个组件
				//调用nowClickFun
				//if(data.liElements[oneSelf.preIndex] != undefined)
				if(oneSelf.preIndex != undefined) 
					oneSelf.preClickFun(oneSelf.preIndex, data.liElements[oneSelf.preIndex]);
				oneSelf.nowClickFun(i, e.target);
				oneSelf.preIndex = i;
			}else{//重复点击入口
